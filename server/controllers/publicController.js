...
module.exports = {
  ...
  feedback: async (req, res, next) => {
    try {
      //form validation
      const { errors, isValid } = validateFeedbackInput(
        req.body
      );
      if (!isValid) {
      ...
      res
        ...
        .json({
          message:
            "Your Feedback Message Sent Successfully.",
        });
    } catch (err) {
      console.log(
        "Error in sending feedback message",
        err.message
      );
      return res
        .status(400)
        .json({
          message: `Error in  sending feedback message ${err.message}`,
        });
    }
  },
};
