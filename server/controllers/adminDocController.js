...
module.exports = {
  uploadDoc: async (req, res, next) => {
    try {
      const { error, isValid } = validateuploadDoc(
        req.body
      );
      if (!isValid) {
      ...
      await cloudinary.uploader.upload(
        ...
        async function (err, result) {
          ...
          return res
            ...
            .json({
              message:
                "Doctor detail with image uploaded Sucessfully",
            });
        }
      );
    } catch (err) {
      ...
      return res
        .status(400)
        .json({
          message: `Error in uploadPost ${err.message}`,
        });
    }
  },
  ...
  leaveDoc: async (req, res, next) => {
    try {
      //validate the form
      const { errors, isValid } = validateLeaveInput(
        req.body
      );
      if (!isValid) {
        return res.status(400).json(errors);
      ...
      if (flag == 0) {
        ...
        return res
          .status(400)
          .json({ message: "already Leave Marked" });
      }
      //success message
      res.status(200).json({
      ...
      return res
        .status(400)
        .json({
          message: `Error in marking leave ${err.message}`,
        });
    }
  },
  ...
  leaveDocCancel: async (req, res, next) => {
    try {
      ...
      return res
        .status(400)
        .json({
          message: `Error in cancelling leave ${err.message}`,
        });
    }
  },
  ...
  deleteDoc: async (req, res, next) => {
    try {
      ...
      console.log(
        "Error in Deleting the doctor account",
        err.message
      );
  ...
  viewAllDoc: async (req, res, next) => {
    try {
      ...
      console.log(
        "Error in Displaying the doctor details",
        err.message
      );
      return res.status(400).json({
