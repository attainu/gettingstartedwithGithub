...
module.exports = {
  appoCheck: async (req, res, next) => {
    try {
      ...
      const docData = await Doctor.findOne({
        department,
        name,
      });
      if (!docData) {
        return res
          .status(400)
          .json({ message: "No such doctor found" });
      }
      console.log(docData);
      ...
      docData.leave.map((date) => {
        if (date == appoDate) {
          return res
            .status(400)
            .json({
              message: `Doctor not available on : ${appoDate}`,
            });
        }
      });
      ...
      if (appoFind.length == 0) {
        return res
          .status(200)
          .json({
            message: "Carry on ur Appoinment Booking",
          });
      }
      ...
      if (appoFind.length == 8) {
        return res
          .status(400)
          .json({
            message: "No sloat avaiable for the given date",
          });
      }
      // checking weather all the slots are filled for date
      if (appoFind.length < 8) {
        return res
          .status(400)
          .json({
            message: "Try another sloat for the given date",
          });
      }
      ...
      appoFind.map((timeSloat) => {
        if (timeSloat.appoTimeSloat == appoTimeSloat) {
          return res
            .status(400)
            .json({ message: "No sloat available" });
        }
      });
      //succes return
      return res
      ...
      console.log(
        "Error in checking appoDate and appoSloat",
        err.message
      );
      return res
        .status(400)
        .json({
          message: `Error in appoDate and appoSloat ${err.message}`,
        });
    }
  },
  ...
  appoBook: async (req, res, next) => {
    try {
      const { errors, isValid } = validateAppoBooking(
        req.body
      );
      if (!isValid) {
      ...
      console.log(
        "Error in adding Paitent Detail",
        err.message
      );
      return res
        .status(400)
        .json({
          message: `Error in adding Paitent Detail ${err.message}`,
        });
    }
  },
    ...
    try {
      ...
      if (!appoData) {
        return res
          ...
          .json({
            message:
              "No record found for given Appoinment ID",
          });
      } else {
      ...
      console.log(
        "Error in Cancelling appoinment",
        err.message
      );
      return res
        .status(400)
        .json({
          message: `Error in Cancelling appoinment" ${err.message}`,
        });
    }
  },
  ...
  appoView: async (req, res, next) => {
    try {
      ...
      if (!appoData) {
        return res
          .status(400)
          .json({ message: "No Appoinment record found" });
      }
      // success message
      ...
      console.log(
        "Error in Viewing appoinment",
        err.message
      );
      return res
        .status(400)
        .json({
          message: `"Error in Viewing appoinment" ${err.message}`,
        });
    }
  },
  ...
  appoAdminView: async (req, res, next) => {
    try {
      ...
      if (!docData) {
        return res
          .status(400)
          .json({ message: "Doctor Acess Denied" });
      }
      // getting doctor id
