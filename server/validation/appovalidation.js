...
const validateAppoBooking = (data) => {
  ...
  data.department = !isEmpty(data.department)
    ? data.department
    : "";
  data.appoDate = !isEmpty(data.appoDate)
    ? data.appoDate
    : "";
  data.appoTimeSloat = !isEmpty(data.appoTimeSloat)
    ? data.appoTimeSloat
    : "";
  ...
  if (
    !Validator.isLength(data.pname, { min: 3, max: 30 })
  ) {
    errors.pname =
      "Patient Name must be between 3 and 30 characters";
  }
  ...
  if (
    !Validator.isLength(data.contact, { min: 10, max: 10 })
  ) {
